...
# 任务:
根据为【达人名称: {influencerName}】在【平台: {platform}】提供的【内容数据列表 (JSON 格式)】，深入分析并生成该达人在该平台的平台账号详情信息。请侧重于分析内容的**主题、风格、受众特征、推广行为和整体调性**。

# 输入数据说明:
Below is the list of content data (in JSON format) for the specified influencer and platform. Analyze this data according to the guidelines.
```json
{content_list_json}
```
Each record in the list contains the following key fields:
*   `content_title`: 帖子的文字内容或视频标题。
*   `promo_category`: 基于爬虫搜索标签得出的推广品类。
*   `enhanced_tag`: 基于爬虫增强标签得出的标签。
*   `cover_image_url`: 封面图链接 (如果AI模型能处理图像，可用于视觉分析)。
*   `content_url`: 原始内容链接 (AI模型通常无法直接访问)。
*   `like_count`: 内容的点赞数。
*   `comment_count`: 内容的评论数。
*   `publish_date`: 内容的发布日期。

**# 输出要求:**
请根据输入数据，分析并生成以下字段的信息，并以**JSON格式**返回结果。对于无法根据现有信息确定的字段，请使用 "无法判断" 或 "信息不足"。
```json
{{
  "audienceGender": "分析的主要受众性别 (例如：男性为主, 女性为主, 均衡, 无法判断)",
  "audienceAge": "分析的主要受众年龄段 (例如：18-24岁, 25-34岁, 跨度较广, 无法判断)",
  "regionCountry": "根据内容语言、话题、评论等推断的主要受众地区/国家 (例如：美国, 东南亚, 全球英语区, 无法判断)",
  "language": "内容主要使用的语言 (例如：英语, 西班牙语, 中文, 无法判断)",
  "contentFormat": ["列出主要的几种内容格式 (例如：开箱测评, 教程, Vlog, 清单推荐, 短剧, 挑战, 直播切片), 基于标题和内容描述推断"],
  "recentContentSummary": "用1-2句话总结最近发布内容的主要特点和主题方向 (重点参考最近3-5条内容)",
  "videoStyle": "描述视频的视觉和剪辑风格 (例如：快节奏, 电影感, 简洁明了, 原始粗糙, 动画/特效多, 无法判断 - 主要基于可推断信息)",
  "contentTone": "描述内容的整体感觉和氛围 (例如：专业严肃, 轻松幽默, 温馨治愈, 高级感, 真实生活流, 可爱活泼, ACG二次元, 无法判断)",
  "categoryDepth": "评估内容是否高度专注于特定领域 (例如：高度垂直(美妆), 中度垂直(家居生活), 泛领域(日常Vlog), 无法判断)",
  "promotionAbility": "评估达人推广产品的能力和频率 (例如：频繁推广/硬广多, 自然植入/软广为主, 测评类推广, 几乎无推广, 无法判断)",
  "brandRepetitionRate": "评估合作品牌的重复情况 (例如：与少数品牌深度绑定, 合作品牌多样/轮换快, 多为单次推广, 未见明显商业推广, 无法判断)",
  "contentScene": ["列出内容中常见的几个主要场景标签 (例如：居家环境, 工作室, 户外旅行, 厨房, 汽车内, 无法判断 - 主要基于可推断信息)"],
  "platform": "平台名称 (例如：youtube, tiktok, ins, 无法判断)"
}}
# 分析指南:
...
# 任务:
根据为 达人ID【{influencerId}】、达人主要名称【{influencerName}】提供的，包含其在各平台详细分析结果的【{platform_details_list_json}】，进行全面的跨平台分析和综合评估，直接生成该达人【达人账号总表】所需的特定字段信息。

# 输入数据说明:
你将收到以下输入变量:
1.  `influencerId`: 需要分析的达人唯一ID。
2.  `influencerName`: 需要分析的达人主要名称。
3.  `platform_details_list_json`: 一个 **JSON格式的字符串**。这个字符串代表一个列表 (list)，列表中的每个对象包含该达人在**一个特定平台**的详细账号指标和分析结果。
